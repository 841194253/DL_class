...
#
# # 读取数据，不进行日期解析
# dataset = pd.read_csv('../exam_latest/dataset/home_electricity/household_power_consumption.txt',
#                       sep=';',
#                       header=0,
#                       low_memory=False,
#                       engine='c')
...
# print(dataset.iloc[dataset.values == '?'].count())
#
# dataset.replace('?', np.nan, inplace=True)
# print(dataset.iloc[dataset.values == '?'].count())
# dataset.isna().sum()
# value = dataset.values.astype('float32')
# dataset['sub_metering_4'] = (value[:,0] * 1000 / 60) - (value[:,4] + value[:,5] + value[:,6])
#
# print(dataset.shape)
...
def split_dataset(data):
    ...
    # 将数据划分为按周为单位的数据
    train = train.reshape((train.shape[0] // 7, 7, train.shape[1]))  # 按7天一组重塑
    test = test.reshape((test.shape[0] // 7, 7, test.shape[1]))  # 按7天一组重塑
...
def cnn_lstm_model(train, sw_width, in_start=0, verbose_set=0, epochs_num=20, batch_size_set=4):
    ...
    model.add(LSTM(200, activation='relu', return_sequences=True,stateful=False))
...
if __name__ == '__main__':
    dataset = pd.read_csv('../exam_latest/dataset/home_electricity/household_power_consumption.csv', header=0,
